...
def extract_objects_from_labelme_data(input_dir, output_dir, margin=10, single_file_name=None):
    """
    ...
    if single_file_name:
        # If a single file name is provided, filter the json files
        labelme_json_paths = [os.path.join(input_dir, single_file_name + '.json')]
        if not os.path.exists(labelme_json_paths[0]):
            raise FileNotFoundError(f"File {labelme_json_paths[0]} does not exist")
